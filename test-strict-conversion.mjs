...
async function testStrictConversion() {
    try {
        console.log('🧪 开始严格版测试');
        
        // 导入严格版转换器
        const { MuPDFConverterStrict } = await import('./src/utils/MuPDFConverterStrict.ts');
        console.log('✅ 成功导入严格版转换器');
        ...
        // 初始化转换器
        const converter = new MuPDFConverterStrict();
        await converter.initialize();
        console.log('✅ 严格版转换器初始化成功');
...
// 运行测试
testStrictConversion().then(success => {
    if (success) {
        console.log('\n🎊 严格版本完全成功！');
        console.log('严格的中文文本提取已经完美解决乱码问题！');
        console.log('生成的PDF和TXT文件只包含真正可读的中文内容！');
    } else {
        console.log('\n💥 严格版本失败，需要进一步调试');
    }
